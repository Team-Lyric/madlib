...
//collect mouse location
  ...
  var templateSelection;
  var genres = ['rock', 'pop', 'randb', 'country'];



  if (divChoice === genres[0] ) {
      //upper left
    templateSelection = 'rock';
  }
  else if (divChoice === genres[1]){
      //upper right corner
    templateSelection = 'pop';
  }
  else if (divChoice === genres[2]){
      //lower left corner
    templateSelection = 'rNb';
  }
  else if (divChoice === genres[3]){
      //lower right corner
    templateSelection = 'country';
  }
  else{
    alert('choose a genre of music');
  }
  var genreChoice = JSON.stringify(templateSelection);
  localStorage.setItem(genreChoice, genreChoice);
}
